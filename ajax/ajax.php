<?php
	...
	$ajax_action = function () {
		...
		$format_of_view  = $options['format_of_view'] ?? 'list';
		$pagination_type = $options['pagination_type'] ?? 'digit';
		...
		$index               = 0;
		$html_output         = '';
		$number_of_all_pages = $query->max_num_pages;
		...
		$pagination_args = array(
			...
			'format'    => '?position=%#%', // should use link to search page nor to ajax (ajax url use if base is empty).
...
// pagination.
		if ( 'digit' == $pagination_type ) {
			...
			$html_output = '<section id="search_result">' . $html_output . $s . '</section>';
		}

		if ( 'more' == $pagination_type ) {// "load more" button pagination.
			if ( $paged == 1 ) {
				$html_output = '<section id="search_result">' . $html_output . $s . '</section>';
			} else {
		...
		$data = array(
			...
			'remove'             => ( $paged == $number_of_all_pages ) ?? null, // need for remove 'load more button' on last page.
			'pagination_type'    => $pagination_type,
			'returned_posts_num' => count( $posts ), //need to defined html replacement routine.
		);
